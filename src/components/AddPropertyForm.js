...
const AddPropertyForm = ({ username }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8090/api/properties/create?username=' + username, formData, {
        headers: {
  ...
  useEffect(() => {
        if (propertyId) {
          const fetchImage = async () => {
            try {
              const imageResponse = await axios.get(`http://localhost:8090/api/properties/image/${propertyId}`, {
